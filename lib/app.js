...
module.exports.initialize = function (config) {
  ...
  app.locals.navBar = '';
  app.locals.navigationFileLocation;
  var retrieveNavigationFilePath = function () {
    var repoDir = config.get("application").repository;
    var subRepoDir = config.get("application").docSubdir;
    var navigationFileName = "navigation.md";
    var navigationFilePath = path.join(repoDir, subRepoDir, navigationFileName);
    console.log('Navigation file location: ' + navigationFilePath);
    app.locals.navigationFileLocation = navigationFilePath;
    return navigationFilePath;
  };

  app.use('/wiki', function (req, res, next) {
    navBar.parse(app.locals.navigationFileLocation || retrieveNavigationFilePath(), function(html) {
